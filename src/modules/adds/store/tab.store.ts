...
const tabStore: StateCreator<AddsTab> = (set, get) => {
	return {
		tabs: [],
		addTab: (payload) => {
			const { tabs } = get();
...
const addsTabs: ReturnType<typeof createTabStore>[] = [];

export const useAddsTab = () => {
	const storageKey = `${localStorage.getItem(TABS.KEY)}`;
	const foundStore = addsTabs.find(
		({ persist }) => persist?.getOptions().name === storageKey,
	);

	if (!foundStore) addsTabs.push(createTabStore());
